...
double evaluatePostfix(const std::string& postfix) {
    ...
    while(iss >> token) {
        if(isOperator(token[0])) {
            ...
            switch(token[0]) {
                case '+': result = Costcalculator::add(a, b); break;
                case '-': result = Costcalculator::subtract(a, b); break;
                case '*': result = Costcalculator::multiply(a, b); break;
                case '/': 
                    if (b == 0) {
                        throw std::invalid_argument("Division by zero is not allowed.");
                    }
                    result = Costcalculator::divide(a, b); break;
            }
